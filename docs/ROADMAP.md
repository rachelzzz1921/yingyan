...
> **最后更新**：2026-06-15 · 与 `prototype/docs/TASKS.md` 对齐
...
| T4-3 | L4 shadow harness 接规则准入 UI（三验 FPR≤10%） | P1 | 治理页显 shadow_metrics |
| T4-4 | L1 自动 spawn eval 或 CI 读 baseline JSON | P1 | G2 report 有通过率 |
| T4-5 | AuditBench 扩至 20 案卷（+5 边界干扰） | P1 | bench 仪表盘 20 行 |
| T4-6 | `yhf gate --strict` 接入 GitHub Actions / 本地 pre-push | P2 | CI 文档 |

**Definition of Done**：`node yhf/gate.js --strict` 且 main recall ✅；看板 YHF 区全绿。
