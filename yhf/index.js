...
const { runPrecheckSmoke } = require('./harness/l7-precheck-smoke');
const { runCitationIntegrityHarness } = require('./harness/l8-citation-integrity');
...
async function runYhfGate(opts = {}) {
  ...
  const strict = opts.strictLayers === true;
  const report = { generated: new Date().toISOString(), mode: 'oracle', overall_pass: true, strict_layers: strict, layers_run: layers.slice() };

  if (layers.includes('engine')) {
    report.engine = runEngineHarness();
    if (!report.engine.gates.G0_clean_zero_fp) report.overall_pass = false;
  }
  if (layers.includes('pipeline') || (!strict && cfg.gates?.G6_pipeline_oracle?.enabled)) {
    report.pipeline = await runPipelineHarness();
    if (report.pipeline.pass === false) report.overall_pass = false;
  }
  if (layers.includes('precheck') || (!strict && cfg.gates?.G7_precheck_smoke?.enabled)) {
    try {
      report.precheck = runPrecheckSmoke();
      if (report.precheck.pass === false) report.overall_pass = false;
    } catch (e) {
      report.precheck = { layer: 'precheck_smoke', status: 'error', message: String(e && e.message), pass: false };
      report.overall_pass = false;
    }
  }
  if (layers.includes('citation') || (!strict && cfg.gates?.G8_citation_integrity?.enabled)) {
    try {
      report.citation = runCitationIntegrityHarness({ minResolvedRate: cfg.citation_min_resolved_rate });
      if (report.citation.pass === false) report.overall_pass = false;
    } catch (e) {
      report.citation = { layer: 'citation_integrity', status: 'error', message: String(e && e.message), pass: false };
      report.overall_pass = false;
    }
  }
  ...
  if (layers.includes('shadow') || (!strict && cfg.gates?.G1_shadow_fpr?.enabled !== false)) {
  ...
  if (layers.includes('gz_production') || (!strict && cfg.gates?.G5_gz_production_ready?.enabled)) {
    try {
