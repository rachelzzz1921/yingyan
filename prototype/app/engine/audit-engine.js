...
const SEVERITY_CODING = {
  '重症肺炎': {
    base: '社区获得性肺炎(普通)', high_group: 'ES31 呼吸系统感染伴重症(高权重)', base_group: 'ES35 呼吸系统感染不伴并发症',
    high_code: 'ES31', base_code: 'ES35', // B2:接分组器/支付标准表,第35条差额真算钱
    criteria: ['呼吸衰竭', '机械通气', '有创通气', '无创(呼吸机|通气)', '感染性?休克', '血管活性', 'ICU', '重症监护', '氧合指数\\s*[（(]?PaO2/FiO2[)）]?\\s*[<≤]\\s*300', 'PaO2/FiO2\\s*[<≤]\\s*300', 'PaO2\\s*[<≤]\\s*60'],
    est_overcoding_amount: 3100, // 支付标准表缺失时的兜底估算
  },
};
...
const ruleCheckers = {
  ...
  'T-205': (ctx) => {
    ...
    if (preventive && !priorSevereNeutropenia) {
      findings.push(mkFinding(ctx, 'T-205', {
        ...
        reasoning: `长效升白针（聚乙二醇化G-CSF）医保限定支付范围为"限前次化疗曾发生重度中性粒细胞减少的患者"。本材料包前次周期仅有入院记录概述性记载"无骨髓抑制相关不适"——但"无症状"不等于"无实验室重度中性粒细胞减少"（重度中性粒减少可无症状），判定"前次是否曾发生重度中性粒细胞减少"须调阅前次住院逐日血常规，而该证据不在本单份材料包内。本次用药时ANC正常、为预防性使用。→ 模式异常但材料包内无法闭环，依"存疑转线索·不误报"出线索（而非疑点），附调阅清单。`,
  ...
  'T-207': (ctx) => {
    ...
    findings.push(mkFinding(ctx, 'T-207', {
      ...
      reasoning: `临时医嘱开立培美曲塞为本次化疗方案（培美曲塞+卡铂+贝伐珠单抗）必需药；病程 ${outsideNote.date} 明确记载"我院药房缺药，嘱家属院外自购"；逐行核对费用清单无任何培美曲塞收费行。培美曲塞为医保目录内、住院必需药，本应由住院医保支付却转患者院外自费购买，涉嫌费用转嫁、规避DRG打包付费。定性证据四方齐备（医嘱+病程+清单缺失+目录属性），但精确转嫁金额须以患者院外自费购药凭证佐证、该凭证不在本材料包内 → 依"存疑转线索·不误报"先出线索并附调阅清单，金额闭环后可升级疑点。`,
  ...
  'D-401': (ctx) => {
    ...
    // B2:第35条差额真算钱——高套组与实际应入组的支付标准差额(支付标准=权重×基准费率)
    let loss = null;
    try { loss = require('./drg-grouper').article35Loss(cfg.high_code, cfg.base_code); } catch (_) { /* 支付标准表缺失→退估算 */ }
    const amount = loss ? loss.loss : cfg.est_overcoding_amount;
    return [mkFinding(ctx, 'D-401', {
      status: '疑点', risk_level: '高', amount_involved: amount,
      evidence: [
        ...
        ...(loss ? [ev('损失测算', `实施细则第35条 · ${loss.version}`, `高套组 ${loss.claimed.drg_code}(权重${loss.claimed.weight},支付标准${money(loss.claimed.standard)}元) − 应入组 ${loss.correct.drg_code}(权重${loss.correct.weight},支付标准${money(loss.correct.standard)}元) = 基金损失 ${money(loss.loss)}元；${loss.formula}`)] : []),
      ],
      reasoning: `病案首页主诊断编「${sev}」拔高入组至「${cfg.high_group}」，但入院记录、逐日病程、血气分析均无重症肺炎诊断标准（无呼吸衰竭/机械通气/休克/ICU，氧合指数>300，CURB-65低危），出院诊断亦为「${cfg.base}」——编码无病历支持且导致入组权重升高，构成高套分组（《条例》38条第(七)项，实施细则第23条："采取高套或低编病种(病组)编码…可认定属38条第七项"）。差异落在CC/MCC升级、确实改变DRG组与权重（非不作为入组条件的编码瑕疵），故构成套高而非质控提示。${loss ? `基金损失依实施细则第35条按两组支付标准差额认定：${loss.formula}（分组依据:${loss.version}${loss.grouper_source === 'opendrg' ? '·OpenDRG分组器' : '·演示子集数据表'}）。` : ''}`,
      needs_more: loss ? [] : ['接入分组器/支付标准表后按实施细则第35条精确认定差额'],
      disposal: `建议按「${cfg.base_group}」重新入组结算，依实施细则第35条退回两组支付标准差额 ${money(amount)} 元${loss ? '' : '（估算，以分组器核定为准）'}。`,
    })];
  },
    ...
    for (const l of record.fee_list.items) {
      ...
      if (age < 18 && quinoloneRe.test(l.item_name)) {
        findings.push(mkFinding(ctx, 'AGE-101', {
          ...
          reasoning: `患者 ${age} 岁（未成年 18 岁档）,费用清单第${l.line_no}行使用喹诺酮类「${l.item_name}」${l.amount ? money(l.amount) + '元' : ''}。喹诺酮类说明书明确 18 岁以下禁用,属超说明书禁忌用药且超限定支付范围（两库第一批药品限儿童/第八批项目限年龄的年龄分层逻辑:未成年用药有 14 岁与 18 岁两个层次,本条命中 18 岁档）。`,
          disposal: l.amount ? `建议全额拒付 ${money(l.amount)} 元并核查用药医嘱合理性,如病历记载特殊获益评估可申诉复核。` : `建议更换为未成年适用抗菌药物;如有特殊获益评估依据请在病历中记载后再开立。`,
        }));
        ...
        findings.push(mkFinding(ctx, 'AGE-101', {
          ...
          reasoning: `患者 ${age} 岁,费用清单第${l.line_no}行使用儿童专用制剂「${l.item_name}」${l.amount ? money(l.amount) + '元' : ''}。该类制剂医保限儿童（14 岁档）支付,≥14 岁使用超限定支付范围（两库第一批药品限儿童）。`,
          disposal: l.amount ? `建议拒付超限定支付部分 ${money(l.amount)} 元。` : `建议更换为该年龄段适用剂型/品规,或走自费告知。`,
        }));
      }
    }
    return findings;
