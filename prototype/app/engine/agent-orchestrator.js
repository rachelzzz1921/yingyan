...
const STAGE_DEFS = [
  {
    ...
    harness: { 输入数据源: '多模态材料包(PDF/扫描件/结构化 JSON/连接器)', 知识源: '40 表字段映射 · 单据槽位定义', 工具: 'PP-Structure OCR · 字段抽取纠错 · pii-redact/llm-privacy-gate(送模前脱敏)', 输出: '结构化 medical_record(每条事实带源锚点)' },
  },
  {
  ...
  },
  {
    ...
    harness: { 输入数据源: '候选疑点 + 材料原文(脱敏副本)', 知识源: '三要素门禁(证据/条款/推理)· 合规前置规则', 工具: '合议去重 reconcile · 定义层仲裁 · CoVe 批量取证自检(可选,脱敏材料包)', 输出: '核验后疑点(缺三要素降级线索·不误报)' },
  },
  {
    no: 5, stage: '调查核实', agent: '对抗合议 Agent(三人格)', kind: 'LLM(按需/预载)',
    harness: { 输入数据源: '仅双方陈述书(信息不对称:辩方见临床/控方见规则/裁定见陈述,均脱敏)', 知识源: 'KB2 深喂 · 曝光台判例库 · 实施细则裁量依据', 工具: 'tri-persona 立论→质证→裁定 · 引用硬校验(引不出条目 ID→转人工)', 输出: '裁定{成立/部分成立/证据不足转人工}+ 评分 + 依据链' },
  },
  {
