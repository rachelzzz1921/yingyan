#!/usr/bin/env node
/**
 * 抓取现场海报用产品截图（需 prototype 已在 localhost:3700 运行）
 */
...
async function capture(page, file, opts = {}) {
  ...
  const legacy = file
    .replace(/^14-workbench-findings/, '13-workbench')
    .replace(/^15-priority-queue/, '25-priority');
  if (legacy !== file) await copyFile(full, path.join(OUT, legacy));
  if (file.includes('14-workbench-findings')) await copyFile(full, path.join(PPT_OUT, '13-workbench.png'));
  if (file.includes('15-priority-queue')) await copyFile(full, path.join(PPT_OUT, '25-priority.png'));
  console.log('✓', full);
}
